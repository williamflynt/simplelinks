...
class EdgeCollection:
    ...
    def __init__(self, *edges: Edge) -> None:
        ...
        # The IDs mapped to keys that are related.
        self.id_table: Dict[int, List[str]] = defaultdict(list)
    ...
    def add(self, *edges: Edge) -> List[Edge]:
        ...
        final = []
        for r in edges:
            if self._get_for_attrs(from_=r.from_, to_=r.to_, edge_type=r.edge_type):
                # This exact edge exists.
                continue
            if r.edge_id is None:
                r.edge_id = self._next_id()
            if r.edge_id in self.id_table:
                raise ValueError(f"cannot add duplicate edge_id: {r.edge_id}")
            self._add(r)
            final.append(r)
        return final
    ...
    def find_by_attrs(
        ...
        *,
        ...
        """Find GraphEdges by attributes."""
        return self._get_for_attrs(from_=from_, to_=to_, edge_type=edge_type)
    ...
    def _add_to_tables(self, r: Edge, idx: int) -> List[str]:
        """Add the edge to our tracking sets."""
        keys = [r.from_.key, r.to_.key]

        if r.edge_type:
            keys.append(self._key_type(r.edge_type))
        if r.edge_id is not None:
            # Do this one last, so we have all the keys when we insert them
            # to the id_table.
            keys.append(self._key_id(r.edge_id))
            self.id_table[r.edge_id].extend(keys)
    ...
    def _delete(self, indexes: Iterable[int]) -> int:
        ...
        self.id_table = defaultdict(list)
        self.edges = []
        self.add(*new_edges)
        return diff_rels
    ...
    def _get_for_attrs_idx(
        ...
        *,
        ...
        buckets = []
        if from_:
            buckets.append(self.attr_idx_map[from_.key])
        if to_:
            buckets.append(self.attr_idx_map[to_.key])
        if edge_type:
            buckets.append(self.attr_idx_map[self._key_type(edge_type)])
...
class Graph:
    ...
    def add_edges_dir(
        ...
        self._add_edges(from_, to_, edge_type=edge_type, directional=True)
    ...
    def _add_edges(
        ...
        for f in from_:
            for t in to_:
                if f == t:
                    continue
                self.edges.add(Edge(f, t, edge_type=edge_type, directed=directional))
